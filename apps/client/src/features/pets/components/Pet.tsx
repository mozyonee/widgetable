...
const actionBarBtnClass =
	'flex items-center justify-center gap-2 px-4 py-2 bg-surface/80 backdrop-blur-sm rounded-xl border border-secondary/20 shadow-sm hover:bg-surface transition-colors disabled:opacity-50 disabled:cursor-not-allowed';
...
const PetPage = () => {
	...
	const actionSpriteUrls = useMemo(
		() => actionCategories.flatMap((c) => c.actions.map((a) => a.sprite).filter(Boolean)) as string[],
		[actionCategories],
	);
	const actionSpritesLoaded = useImagesLoaded(actionSpriteUrls);
	...
	return (
		<>
			...
			>
				{/* Header */}
				<div className="grid grid-cols-[1fr_auto_1fr] items-center w-full p-4 flex-shrink-0 bg-surface backdrop-blur-sm rounded-b-2xl shadow-md border-b border-secondary/20">
