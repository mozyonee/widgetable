...
export class AuthService {
	...
	getCookieForLogout() {
		...
		return `Authentication=; Path=/; Max-Age=0; SameSite=None;${secure ? ' Secure; HttpOnly; Partitioned;' : ''}`;
	}
	...
	private getCookieWithJwtToken(token: string) {
		const secure = this.configService.get('NODE_ENV') === 'production';
		return `Authentication=${token}; Path=/; Max-Age=${this.configService.get<string>('JWT_EXPIRATION_TIME', '86400')}; SameSite=None;${secure ? ' Secure; HttpOnly; Partitioned;' : ''}`;
	}
}
